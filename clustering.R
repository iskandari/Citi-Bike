...
cluster_vk$cluster <- fit.km[1]

centers_2015 <- as.data.frame(fit.km$centers)  

centers_2015$cluster <- rownames(centers_2015)
...
melted <- melt(fit.km$centers)



png("cluster_lines_vk.png", width = 12, height = 8, units = 'in', res = 300)

ggplot(data=melted, aes(x=variable, y=value, color = cluster, group = cluster)) + geom_line(stat = "identity") +
...
png("cluster_lines_2015.png", width = 10, height = 5, units = 'in', res = 300)
  ...
  ggtitle("Mean centers of hourly availability 2015") +
  xlab("hour") +
  ylab("load factor") +
